...
def ytmusic():
    ...
    filename = f"{'location_path_where_to_store'+video_info['title']}.mp3"
    options={
...
def ytvideo():
    song_name = input('Enter the video name\n').replace(" ","%")
    url = "https://youtube.googleapis.com/youtube/v3/search?part=snippet&q="+song_name+"&key=AIzaSyBo6h-t4F2M_MahpexHJPSS4wiycIm638I&maxResults=5"
    r = requests.get(url).text
    soup = BeautifulSoup(r, 'html.parser')
    ...
    video_url = (str("https://www.youtube.com/watch?v="+video_id))
    video_info = youtube_dl.YoutubeDL().extract_info(url = video_url,download=False)
    filename = f"{'location_path_where_to_store'+video_info['title']}.mp4"
    options={
        'outtmpl':filename,
    }
